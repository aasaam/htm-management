    ...
    <v-card :elevation="$vuetify.theme.dark ? 9 : 8">
      ...
      >
        <template v-if="generalAction.deletable" v-slot:top>
          <ListDialog
            :dialog.sync="dialog"
            :title="$t('deleteUpstream')"
            :okbtn="$t('deleteUpstream')"
            :closebtn="$t('nope')"
            :done-event="deleteUpstream"
        ...
        <template v-slot:[`item.loadBalanceMethod`]="{ item }">
          <span v-if="item.loadBalanceMethod === 'HI'">
            {{ $t('hashIp') }}
          </span>
          <span v-if="item.loadBalanceMethod === 'CO'">
            {{ $t('cookie') }}
          </span>
          <span v-if="item.loadBalanceMethod === 'RR'">
            {{ $t('roundRobin') }}
          </span>
        </template>
    ...
    params: {
      handler(after, before) {
        ...
        this.debounce = setTimeout(() => {
          ...
          this.callMeFetch('upstream/list/listUpstream');
        }, 800);
      },
      deep: true,
    },
  ...
  methods: {
    ...
    async deleteUpstream() {
      ...
      const [err, data] = await to(
        this.$store.dispatch('upstream/edit/deleteUpstream', _id),
      );
      if (data) {
